...
export function AdminGate({ children }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
      <div
        className={`w-full max-w-sm bg-white rounded-2xl border border-gray-200 shadow-lg p-8 transition-transform ${shaking ? "animate-[shake_0.4s_ease-in-out]" : ""}`}
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full h-11 rounded-xl bg-primary text-white font-semibold text-sm hover:bg-primary/90 active:scale-[0.98] transition-all">
            Unlock Admin Panel
          </button>
        </form>
      </div>
